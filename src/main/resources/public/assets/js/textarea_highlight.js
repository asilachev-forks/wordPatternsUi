    ...
    if (optionsHigh.eleId != 'undefined') {
        ...
        if (conn.length) {
            if (placer != 'undefined') {
                ...
                //default options:
                if (typeof optionsHigh.width == 'undefined') {
                    optionsHigh.width = 300;
                }
                if (typeof optionsHigh.height == 'undefined') {
                    optionsHigh.height = 100;
                }
                if (typeof optionsHigh.padding == 'undefined') {
                    optionsHigh.padding = 5;
                }
                if (typeof optionsHigh.fontFamilly == 'undefined') {
                    optionsHigh.fontFamilly = 'Arial';
                }
                if (typeof optionsHigh.background == 'undefined') {
                    optionsHigh.background = 'white';
                }
                if (typeof optionsHigh.radius == 'undefined') {
                    optionsHigh.radius = 5;
                }
                if (typeof optionsHigh.fontSize == 'undefined') {
                    optionsHigh.fontSize = 13;
                }
                if (typeof optionsHigh.lineHeight == 'undefined') {
                    optionsHigh.lineHeight = optionsHigh.fontSize + 5;
                }
                if (typeof optionsHigh.backgroundControls == 'undefined') {
                    optionsHigh.backgroundControls = "#151515";
                }
                if (typeof optionsHigh.counterFont == 'undefined') {
                    optionsHigh.counterFont = 'black';
                }

                //if conter for letters is set create element:
                if (typeof optionsHigh.counterlettres != 'undefined') {
                    if (optionsHigh.counterlettres != false) {
                        $(conn).append('<div id="counter_' + placer + '">Chars count: 0</div>')
                ...
                $(conn).css({
                    ...
                    'border': '0px',
                    'width': (optionsHigh.width + 30) + 'px',
                    'height': (optionsHigh.height + 28) + 'px'
                });
                $(connTextArea).css({
                    'direction': 'ltr',
                    'position': 'absolute',
                ...
                $(connTextArea).keyup(function(e) {
                    ...
                    var span_style = "padding:0; height:" + (optionsHigh.fontSize + 3) +
                        "px; margin:0; font-size:" +
                        optionsHigh.fontSize + "px; font-family: " +
                        optionsHigh.fontFamilly +
                        "; white-space:pre-wrap; color:white; border-radius:3px; background-color:";
                    ...
                    for (var i = 0; i < optionsHigh.matchpatterns.length; i++) {
                        ...
                        if (typeof optionsHigh.matchpatterns[i][1] != 'undefined') var replace = optionsHigh.matchpatterns[i][1];
                        else var replace = "$1";
                        if (typeof optionsHigh.hightlightsColor != 'undefined') {
                            if (typeof optionsHigh.hightlightsColor[i] != 'undefined') {
                                var color_high = optionsHigh.hightlightsColor[i];
                            } else {
                                var color_high = 'yellow';
                            }
                        } else {
                            var color_high = 'yellow';
                        }
                            ...
                            if (typeof optionsHigh.objectsCopy[i] != 'undefined') {
                                ...
                                if (res != null) {
                                    for (var k = 0; k < res.length; k++) {
                                        strres = strres + ' ' + res[k];
                                        if (k == res.length - 1) {
                                            $('#' + optionsHigh.objectsCopy[i]).text(strres);
                                        }
                                    }
                                }
                            }
                ...
                $(conn).text('please ad aunique placer value');
            }
        } else {
            console.log('no container <div> found please pass the correct id name');
        }
    } else {
        console.log('no container set please pass the id threw - create_bind_textarea_highlight({eleId: ID}); ');
    }
}
